    ...
    if isinstance(func(0.0, 0.0, 0.0), (bool)):
        ...
        points = np.asarray(
            [
                p
                for p in points
                if same_dbl(func(space[p][0], space[p][1], space[p][2]), 0.0, 10)
            ]
        )
    else:
...
def clockwise(space, points):
    return (
        False
        if (
            space[points[0]][0] * space[points[1]][1]
            - space[points[1]][0] * space[points[0]][1]
            >= 0.0
        )
        else True
    )
...
ncons = len(arg) - 4
if ncons < 1:
    sys.exit("You need at least one constraint!")
fout = open(arg[-1], "w")
