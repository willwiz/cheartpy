...
parser.add_argument('--export-master', action='store_true',
                   help='OPTIONAL: whether to export the rectangular mesh on the master domain')
parser.add_argument('dq', help='the angle spanning the arc',
                    type=float)
parser.add_argument('q0', help='where the mesh starts counter clockwise',
                    type=float)
...
def create_cylinder(g: make_grid, args) -> None:
  g.pspace = scaled_cyl_to_cart(g.space, np.array([2.0*pi*args.dq, 2.0*pi*args.q0, args.dr, args.r0]))
