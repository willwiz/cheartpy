...
if not (list_dt1_unscaled.size == n1): raise ValueError('For some reason the number of steps in the transition {} is not the same as requested {}'.format(list_dt1_unscaled.size, n1))
t_unscaled        = list_dt1_unscaled.sum() + n2 # Compute the unscaled remaining time
dt2               = t_remaining/t_unscaled
if not (dt2 > 1e-12): raise ValueError('Something went wrong! But your initial step size are too large!')
dtf = dt2 + dt0
...
print('    The second part has {} steps and {} time elapsed'.format(list_dt1.size, list_dt1.sum()))
