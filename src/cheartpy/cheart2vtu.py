...
settinggroup.add_argument('--no-compression', dest='compress',action='store_false',
...
indexgroup.add_argument('--index',         '-i', nargs=3, dest='irange',    action='store', default=[0,0,1],
...
indexgroup.add_argument('--use-subiter',   '-si', dest='subiter', nargs=3, action='store', default=None,
...
indexgroup.add_argument('--use-subauto',   '-sa', dest='subauto', action='store_true',
...
def print_input_info(inp:InputArgs) -> None:
  ...
  print("The script will be ran through the following range", [inp.index[0], inp.index[-1]])
...
def check_variables(inp : InputArgs) -> None:
  ...
  for var in inp.vars:
    if os.path.isfile(os.path.join(inp.infolder, f"{var}-{inp.index[0]}{cheartsuffix}")):
      ...
      inp.varstemp.append(lambda i: os.path.join(inp.infolder, f"{var}-{i}{cheartsuffix}"))
    elif os.path.isfile(os.path.join(inp.infolder, f"{var}-{inp.index[0]}{cheartsuffix}.gz")):
      inp.varstype.append(CheartDataFormat.zip)
      inp.varstemp.append(lambda i: os.path.join(inp.infolder, f"{var}-{i}{cheartsuffix}.gz"))
    else:
