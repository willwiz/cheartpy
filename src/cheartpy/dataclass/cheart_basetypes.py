...
class HexahedralBasis(Basis):
  def __init__(self, name:str, type:Literal["NODAL_LAGRANGE#", "NL#", "MODAL_BASIS#", "PNODAL_BASIS#", "MINI_BASIS#",
                "NURBS_BASIS#", "SPECTRAL_BASIS#"], method:Literal['GAUSS_LEGENDRE#', 'GL#']) -> None:
    Basis.__init__(self, name, "HEXAHEDRAL_ELEMENT", type, method)

class TETRAHEDRALBasis(Basis):
  def __init__(self, name:str, type:Literal["NODAL_LAGRANGE#", "NL#", "MODAL_BASIS#", "PNODAL_BASIS#", "MINI_BASIS#",
                "NURBS_BASIS#", "SPECTRAL_BASIS#"], method:Literal['GAUSS_LEGENDRE#', 'GL#', 'KEAST_LYNESS#', 'KL#']) -> None:
...
class Variable:
  ...
  format:VariableExportFormat|Literal["TXT", "BINARY", "MMAP"] = VariableExportFormat.TXT
  freq:int = 1
  loop_step:Optional[int] = None
  setting : List[Setting] = field(default_factory=list)
  def AddSetting(self,
      task:Literal["INIT_EXPR", "TEMPORAL_UPDATE_EXPR", "TEMPORAL_UPDATE_FILE", "TEMPORAL_UPDATE_FILE_LOOP"],
  ...
  def write(self, f:TextIO):
    ...
    for s in self.setting:
      if self.loop_step is None:
        f.write(f'  !SetVariablePointer={{{self.name}|{s.name}|{s.value[0]}}}\n')
      else:
        f.write(f'  !SetVariablePointer={{{self.name}|{s.name}|{s.value[0]}|{self.freq}|{self.loop_step}}}\n')
