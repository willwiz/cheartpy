...
def get_element_info(type: str, dim: int):
    if type == "T3D2":
        el = [1, 2]
        if not (dim == 3):
            raise ValueError(
...
Cmesh = TypeVar("Cmesh", bound="cheart_mesh")
...
class Cmesh(object):
    ...
    def import_space(
        self: Cmesh, space: meshtype_space, elmap: Optional[dict] = None
    ) -> Cmesh:
...
def abaqus_importer(f: TextIO) -> list:
    space = None
    elems = newdict()
    reader = None
    for line in f:
...
def main_example(args=None) -> None:
    args = parser.parse_args(args=args)
    if args.o == None:
        ...
        name = args.o
    with open(args.input, "r") as f:
        space, elems = abaqus_importer(f)
    elmap = build_elmap(space=space, topology=elems["Volume1"])
    print(f"<<<Creating mesh from space")
    g = cheart_mesh().import_space(space=space, elmap=elmap)
    print(f"<<<Importing topology from {'Volume1'}")
    top = make_topology(g, elem=elems["Volume1"])
    g.import_topology(top)
    pads = meshtype_boundary("pads")
    print(f"<<<Creating boundary from:")
    for i in [3, 4, 5, 6, 7, 8, 9, 10, 11]:
        string = f"Surface{i}"
        print(f"       {string}")
        pads = pads + make_boundary(g, elems[string], 3)
    print(f"<<<Creating boundary from:")
    for i in [13, 14, 15, 16, 17, 18, 19, 20, 21]:
        string = f"Surface{i}"
        print(f"       {string}")
        pads = pads + make_boundary(g, elems[string], 4)
    print(f"<<<Creating boundary from:")
    for i in [23, 24, 25, 26, 27, 28, 29, 30, 31]:
        string = f"Surface{i}"
        print(f"       {string}")
        pads = pads + make_boundary(g, elems[string], 5)
    print(f"<<<Creating boundary from:")
    for i in [33, 34, 35, 36, 37, 38, 39, 40, 41]:
        string = f"Surface{i}"
        print(f"       {string}")
        pads = pads + make_boundary(g, elems[string], 6)
    print(f"<<<Importing created boundary")
    g.import_boundary(pads)
    print(f"<<<Writing mesh to Cheart files")
    CHWrite_d_utf(name + "_FE.X", g.space)
    CHWrite_t_utf(name + "_FE.T", g.topology, g.nelem, g.nnode)
    CHWrite_iarr_utf(name + "_FE.B", g.boundary)
