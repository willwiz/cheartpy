...
def add_number_to_negative(left: float, right: NegativeTrait) -> float | ExpressionTrait:
    return sub_number_to_(left, right.value)
...
def sub_number_to_expression_sub_(left: float, right: ExpressionTrait) -> ExpressionTrait:
    if right.op != MathOperator.SUB:
        msg = (
            "sub_number_to_expression_sub_ can only handle subtraction expressions. "
            "Use sub_number_to_expression instead."
        )
        raise ValueError(msg)
    if isinstance(right.left, float):
        return Expression(left - right.left, MathOperator.SUB, right.right)
...
def sub_number_to_expression_mul_(left: float, right: ExpressionTrait) -> ExpressionTrait:
    if right.op != MathOperator.MUL:
        msg = (
            "sub_number_to_expression_mul_ can only handle multiplication expressions. "
            "Use sub_number_to_expression instead."
        )
        raise ValueError(msg)
    if isinstance(right.left, float):
...
def sub_number_to_negative(left: float, right: NegativeTrait) -> ExpressionTrait:
...
def sub_number_to_(left: float, right: ALL_TYPES) -> float | ExpressionTrait:
    match right:
        ...
        case ExpressionTrait():
            return sub_number_to_expression(left, right)
        case NegativeTrait():
            return sub_number_to_negative(left, right)
